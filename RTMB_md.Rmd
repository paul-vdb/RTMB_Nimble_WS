...
dlognorm <- function(x, mean, sigma){
  ans <- NULL
  for( i in seq_along(x) ) ans <- c(ans, dnorm(logtransform(x[i]), mean, sigma)*det(logtransform$jacobian(x[i])))
  return(ans)
}
y <- seq(0, 10, by = 0.1)
plot(y, dlognorm(x=y, 0, 1), xlab = "y", ylab = "density - lognormal")

## Logit Transform
logittransform <- MakeTape(function(x){return(log(x/(1-x)))}, 0) ## go from (0,1) -> (-inf, inf)
dlogitnorm <- function(x, mean, sigma){
  ans <- NULL
  for( i in seq_along(x)) ans <- c(ans, dnorm(logittransform(x[i]), mean, sigma)*abs(det(logittransform$jacobian(x[i]))))
  return(ans)
}
p <- seq(0.0001,0.999,0.01)
plot(p, dlogitnorm(x=p, 0, 1), xlab = "p", ylab = "density - logitnormal")
```
...
- If you know the derivative then you can use it instead of AD. This is important for Lambert W as we compute it numerically but have an analytically solution to the derivative!
...
fn <- function(par){
  ...
  Smsy <- (1 - LambertW0(exp(1 - log(alpha))))/beta
  umsy <- (1 - LambertW0(exp(1 - log(alpha))))
  Sgen <-  -1/beta*LambertW0(-beta*Smsy/alpha)
